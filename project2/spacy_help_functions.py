...
def extract_relations(X, doc, spanbert, entities_of_interest=None, internal_name=None, conf=0.7):
    ...
    for sentence in doc.sents:
        ...
        for ep in entity_pairs:
            # not run SpanBERT over entity pairs that do not contain named entities of the right type for the relation of interest r.
        ...
        for ex, pred in list(zip(examples, preds)):
            ...
            #     continue
            # extract all instances of the relation specified by input parameter r
            if relation != internal_name:
                continue
            if not flag:
            ...
            obj = ex["obj"][0]
            confidence = pred[1]
            # print("\t\tRelation: {} (Confidence: {:.3f})\n\t\tSubject: {}\n\t\tObject: {}".format(relation, confidence, subj, obj))
            print("\t\tOutput Confidence: {:.8f} ; Subject: {} ; Object: {} ;".format(confidence, subj, obj))
            if confidence > conf:
